...
(async () => {
  ...
  try {
    ...
    ok(/подставлены в форму|Браузер не запущен|Не удалось заполнить/.test(filled), "ответ vaultFill назвал, что произошло: " + oneLine(filled));
    // НАХОДКА (записана в AGENT-NOTES, правится в фазе поиска ошибок): «Браузер не запущен…»
    // не начинается с «Ошибка», а vault.fillLogin ловит только этот префикс. Пока браузер
    // не открыт, инструмент может ответить «подставлены в форму», хотя заполнения не было.
    if (/^OK — логин и пароль/.test(filled)) {
      info("НАХОДКА: браузер не запущен, а vaultFill отчитался об успехе — записано, правится отдельно.");
    }
