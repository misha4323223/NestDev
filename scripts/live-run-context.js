...
(async () => {
  ...
  const r1 = await run1;
  // «Стоп» посреди ответа модели — мягкая остановка, а не сбой: работа на диске
  // цела, а в окно уходит признак продолжения (там загорается «▶ Продолжить»).
  // Раньше здесь падало «⏹ Генерация остановлена», и человек продолжал руками.
  ok(r1 && r1.ok === true, "«Стоп» завершил прогон мягко, а не ошибкой: " + JSON.stringify(r1));
  const stopNotice = events.slice(beforeStop).filter((e) => e.ev && e.ev.type === "resume").slice(-1)[0];
  ok(
    !!stopNotice && /остановлено пользователем/.test(stopNotice.ev.reason),
    "«Стоп» зажёг кнопку «▶ Продолжить»: " + JSON.stringify(stopNotice && stopNotice.ev.reason)
  );
  ok(!events.slice(beforeStop).some((e) => e.ev && e.ev.type === "error"), "остановка человека ушла в чат ошибкой");
