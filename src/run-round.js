...
function createRunRound(deps) {
  ...
  const run = async (round) => {
    ...
    await consumeProviderStream({
      ...
      // Модель, упёршаяся в лимит вывода, обрывает ответ на полуслове. Раньше это
      // выглядело как законченный ответ: транспорт сообщал об обрыве только у Ollama
      // (done_reason), а у облачных провайдеров — нет (теперь читается и finish_reason).
      onTruncated: () => {
        truncated = true;
      },
      onUsage: (u) => {
