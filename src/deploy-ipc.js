...
async function runCloudDeploy(dir, appName, opts) {
  ...
  const res = await engine.deploy(workDir, {
    ...
    // Секреты: значения приходят из интерфейса или скрипта (не из чата), а в
    // ревизию уезжает ссылка на Lockbox — значение подставляет облако.
    secrets: o.secrets,
    secretId: o.secretId,
    secretKeys: o.secretKeys,
    memoryMb: o.memoryMb,
    cores: o.cores,
    ...
    // Куда выкатываем: в production упавшие тесты останавливают выкат, а для
    // frontend браузерная проверка обязательна. Обход — только явными флагами,
    // и он остаётся в замечаниях и в истории деплоя.
    environment: o.environment,
    allowFailingTests: o.allowFailingTests,
    testsGate: o.testsGate,
    browserCheck: o.browserCheck,
