...
(function (root, factory) {
  ...
  async function renderMobileStatus() {
    ...
    try {
      ...
      // Шифрование: страница телефона едет по https с самоподписанным сертификатом.
      // Человеку надо сказать про предупреждение браузера (иначе решит, что «сломалось»)
      // и тем более надо сказать, если шифрования нет вовсе.
      // Строго по значению поля: если главный процесс про TLS ничего не сказал
      // (старый мост), панель не должна выдумывать ни «зашифровано», ни «открыто».
      if (st.enabled && st.running && st.tls === true) {
        const lock = document.createElement("div");
        lock.className = "mobile-url-none";
        lock.textContent =
          "🔒 Шифрование включено (сертификат самоподписанный). Телефон при первом входе покажет предупреждение — «Дополнительно» → «Перейти на сайт».";
        urls.appendChild(lock);
      }
      if (st.enabled && st.running && st.tls === false) {
        const open = document.createElement("div");
        open.className = "mobile-url-none";
        open.textContent =
          "⚠ Мост работает без шифрования: " + (st.tlsError || "сертификат не создан") +
          ". В открытой сети страницу телефона может подменить кто угодно — подключитесь к своей домашней сети.";
        urls.appendChild(open);
      }
