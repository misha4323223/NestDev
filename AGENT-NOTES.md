...
Основной цикл раундов внутри `runAi` (подготовка запроса, авто-миссия, обработка сбоя с переключением
OpenAI-профиля), блок «Настройки», «Выполнение инструментов», реестр агентских инструментов, превью,
проекты, откат, GitHub OAuth, updater. Дальше по плану — `browser-tools.js`, `agent-tools.js`,
`agent-store.js`, `yandex-cloud.js`, `deploy-engine.js`; `smoke.test.js` — последним.
...
Основной цикл раундов внутри `runAi` (подготовка запроса, авто-миссия, обработка сбоя с переключением
OpenAI-профиля), `buildProjectBrief`, блок «Настройки», «Выполнение инструментов», реестр агентских
инструментов, превью, проекты, откат, GitHub OAuth, updater. Дальше по плану — `browser-tools.js`,
